...
waveform with HiFi-GAN. The kNN step is pure numpy, with no ONNX at match
time.
...
|-----|------|---------|-------------|
| `quantized` | `bool` | `False` | Load INT8 `*_q8.onnx` models. Reduces memory to ~123 MB total, at a slight quality cost. |
...
among all voiceclonnx engines. Slight quality degradation is expected.
...
**First run is slow.** WavLM (about 387 MB) downloads from HF Hub on first
use and is cached in `~/.cache/huggingface/hub`.
...
---
[← bicodec](bicodec.md) · [Home](../index.md) · [focalcodec →](focalcodec.md)
