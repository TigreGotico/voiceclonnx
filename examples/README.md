...
|---|---|---|
| [`basic_clone.py`](basic_clone.py) | Facade demo with knnvc: synthesise two TTS clips via edge-tts and run voice conversion | `vconnx[knnvc]`, `edge-tts` |
| [`cli_batch.sh`](cli_batch.sh) | Bash loop — convert a folder of WAVs using the `vconnx clone` CLI | any engine installed |
| [`quantized_low_memory.py`](quantized_low_memory.py) | INT8 quantized variants for knnvc and openvoice: timing + memory comparison | `vconnx[knnvc]`, `vconnx[openvoice]` |
