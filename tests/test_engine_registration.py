...
missing from the auto-import list in ``voiceclonnx/__init__.py`` (mock tests import
adapter modules directly, which hides the gap).
"""
from pathlib import Path

ENGINES_DIR = Path(__file__).resolve().parent.parent / "voiceclonnx" / "engines"
NON_ENGINE_MODULES = {"__init__", "base"}


def test_every_engine_module_is_registered():
    import voiceclonnx  # noqa: F401 — triggers auto-imports
    ...
    assert not missing, (
        ...
        f"voiceclonnx/__init__.py): {sorted(missing)}"
    )
